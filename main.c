...
int main(void)
{
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
      ...
      HAL_UART_Transmit(&huart2, (uint8_t*)buffer, strlen(buffer), HAL_MAX_DELAY);  //Transmit the average reading via UART
      ...
      if (average > 0) {  // Ensure the average is not zero to avoid division errors
          ...
          HAL_UART_Transmit(&huart2, (uint8_t*)buffer, strlen(buffer), HAL_MAX_DELAY);  // Transmit the resistance value via UART
          ...
          HAL_UART_Transmit(&huart2, (uint8_t*)buffer, strlen(buffer), HAL_MAX_DELAY);  // Transmit the temperature value via UART
      } else {
          sprintf(buffer, "Error: Invalid ADC reading\r\n");  // Handle invalid ADC reading (e.g., division by zero)
          HAL_UART_Transmit(&huart2, (uint8_t*)buffer, strlen(buffer), HAL_MAX_DELAY);  // Transmit the error message via UART
      }
